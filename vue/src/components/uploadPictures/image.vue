<template>
    <div>
        <div v-if="maxlength == 1">
                ...
                <template #trigger="{ open }">
                    <div class="image-container" v-if="url" @mouseenter="onMouseenter" @mouseleave="showIcons = false">
                        ...
                        <div v-if="showIcons && opt" class="icon-container">
                            <t-space>
                                <span class="icon-preview" @click="open">
                                    <BrowseIcon />查看
                                </span>
                                <span class="icon-delete" @click="deleteImage">
                                    <DeleteIcon />删除
                                </span>
                            </t-space>
                        </div>
            ...
            <t-image v-else :src="url" @click="modalPicTap(maxlength, title)" :style="style"
                :error="renderAddCustomIcon" />
        </div>
        <div v-else style="display: flex;gap:15px">
            <div v-for="(url, index) in srcs" :key="index" class="srcs_image">
                <t-image :src="url" :style="style" :error="renderCustomIcon" :data-index="index"
            ...
            <!-- 数量小于最大个数 默认添加 -->
            <t-image v-if="srcs.length < maxlength" @click="modalPicTap(maxlength, title)" :style="style"
                :error="renderAddCustomIcon" />

        </div>
        <t-dialog v-model:visible="modalPic" width="950px" :header="title" :closeOnOverlayClick="false" :footer="false">
            <uploadPictures :isChoice="isChoice" @getPic="getPic" @dialogStatus="dialogStatus" :maxlength="maxlength">
            </uploadPictures>
        </t-dialog>
    </div>
</template>
...
watch(() => props.urls, (newVal, oldVal) => {
