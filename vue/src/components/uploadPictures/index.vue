<template>
  <div class="Modal">
                ...
                <template #label="{ node }">
                  <div class="nodes" @click="appendBtn(node.data.id)">
                    <span class="file-name">
                      <img src="data:image/jpeg;base64," class="icon" />
                      <span class="name line1">{{ node.label }}</span>
                    </span>

                    <t-dropdown v-if="node.data.id" :options="clickMenuOptions" @click="(data) => clickMenu(data, node.data)">
                      <t-icon name="ellipsis" size="16" />
                    </t-dropdown>
                    <t-link v-else theme="default" size="small" @click="append(node.data, 'add')" variant="text" class="add">添加</t-link>
                  </div>
                </template>
              </t-tree>
            </div>
      ...
      <div class="conter">
        <div class="bnt acea-row row-middle df-jcsb" style="display: flex">
          <div class="" style="flex: 1">
            <t-space>
              <t-button theme="primary" :disabled="checkPicList.length === 0" @click="checkPics" v-if="isShow !== 0">使用选中图片</t-button>
              <uploadImg
                app="admin"
                :data="{
                  type: 'image',
              ...
              <t-button theme="default" class="mr10" :disabled="!checkPicList.length && !ids.length" @click="editPicList()">删除图片</t-button>
            </t-space>
          </div>
          <div v-if="isPage" style="display: flex">
            <t-input class="mr10" v-model="fileData.name" placeholder="请输入图片名" size="small" style="width: 150px">
              <template #suffixIcon>
        ...
        <div class="pictrueList acea-row" :class="{ 'is-modal': !isPage }">
          <div style="width: 100%">
            ...
            <div v-else class="acea-row mb10">
              <div class="pictrueList_pic mb10 mt10" v-for="(item, index) in pictrueList" :key="index" :style="{ margin: picmargin }" @mouseenter="enterMouse(item)" @mouseleave="enterLeave(item)">
                ...
                <div class="operate-item">
                  ...
                  <t-input size="small" type="text" v-model="item.name" v-else @blur="bindTxt(item)" />
                  <div class="operate-height">
                    <t-space>
                      <span class="operate mr10" @click="editPicList(item.id)" v-if="item.isShowEdit">删除</span>
                      <span class="operate mr10" @click="item.isEdit = !item.isEdit" v-if="item.isShowEdit">改名</span>
                      <t-image-viewer v-model:visible="lookImgView" :images="currentPicList" :default-index="lookImgIndex">
                        <template #trigger>
                          <span class="operate" @click="lookImg(item)" :images="currentPicList" :index="index" v-if="item.isShowEdit">查看</span>
                        </template>
                      </t-image-viewer>
                    </t-space>
                  </div>
        ...
        <div class="footer acea-row row-right">
          <t-pagination v-if="pagination.total > pagination.defaultPageSize" v-model="pagination.defaultCurrent" v-model:pageSize="pagination.defaultPageSize" :total="pagination.total" :show-page-size="false" :total-content="false" style="margin-top: 10px" @current-change="getFileList" />
        </div>
      </div>
    </div>
    ...
    <!-- 新建/编辑分类 -->
    <t-dialog v-model:visible="listDialogVisible" :close-on-overlay-click="false" :destroy-on-close="true" :header="listDialogTitle" :on-confirm="onListSubmit">
      <t-form ref="formRef" label-align="left" :data="listFormData" :rules="FORM_RULES" :label-width="80">
        <t-form-item label="上级分类" name="pid">
          <t-tree-select
            v-model="listFormData.pid"
            placeholder="请选择上级分类"
            :data="treeData"
            clearable
            filterable
...
import { getCategoryListApi, createApi, fileListApi, categoryEditApi, moveApi, fileUpdateApi } from '@/api/admin/common/uploadPictures';
...
const checkPics = () => {
  if (props.isChoice === '单选') {
    ...
    if (props.maxlength != undefined && checkPicList.value.length > Number(props.maxlength)) return MessagePlugin.warning('最多只能选' + props.maxlength + '张图片');
