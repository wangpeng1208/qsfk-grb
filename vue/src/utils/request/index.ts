...
import { useAdminUserStore } from '@/store';
...
const host = import.meta.env.MODE === 'release' ? window.location.origin : import.meta.env.VITE_API_URL;
// 数据处理，方便区分多种处理方式
const transform: AxiosTransform = {
  ...
  responseInterceptors: (res) => {
    // 如果响应里的data存在，且code为1，说明是正常业务 ，需要对 data进行解密
  ...
  responseInterceptorsCatch: async (error: any, instance: AxiosInstance) => {
    // 如果api接口错误 会在这里捕获
    // console.log('error', error); 如果error是网络错误
    ...
    if (error.response) {
      ...
      if (status === 401) {
        // MessagePlugin.error('登录失效，请重新登录');
        // 清空token
        // 获取当前应用
        const adminUserStore = useAdminUserStore();
        // userStore为默认应用
        const appStore = adminUserStore;
        appStore.logout();
      }
    }
