...
export const useSettingStore = defineStore('setting', {
  ...
  getters: {
    ...
    showSidebarLogo: (state) => state.layout === 'side' || state.layout === 'sub',
    showHeaderLogo: (state) => state.layout !== 'side' && state.layout != 'sub',
  ...
  actions: {
    async loadConfig() {
      const hasUserPanelConfig =  localStorage.getItem('hasUserPanelConfig');
      if(hasUserPanelConfig) {
        return;
      }
      const response =  await axios.post('/static/config/style.json');
      const config = {
        showFooter: true,
      ...
      // 循环 config 中的 key，更新 store 中的值
      for (const key in response.data) {
        if (config[key] !== undefined) {
          // 分别打印 key 和 config[key]，以便查看
          // console.log(key, config[key]);
          this[key as TStateKey] = config[key as TStateKey];
        }
      }
      // 重新存储state
      this.$patch(state);
    ...
    updateConfig(payload: Partial<TState>) {
      ...
      // 记录 用户有自定义配置
      localStorage.setItem('hasUserPanelConfig', 'true');
    },
