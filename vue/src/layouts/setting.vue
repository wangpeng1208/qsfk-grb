  ...
  <t-drawer v-model:visible="showSettingPanel" size="408px" :footer="false" header="页面配置" :close-btn="true" class="setting-drawer-container" @close-btn-click="handleCloseDrawer">
    <div class="setting-container">
      <t-form ref="form" :data="formData" label-align="left">
        ...
        <t-form-item v-show="formData.layout != 'top'" label="侧边栏模式" name="sideMode">
        ...
        <t-form-item v-show="formData.layout != 'top'" label="侧边栏宽度" name="sidenavWidth">
          <t-slider v-model="formData.sidenavWidth" :min="160" :max="300" />
        </t-form-item>
        ...
        <t-divider lass="setting-group-title">元素开关</t-divider>

        <t-form-item label="只展开一个一级菜单" name="expandMutex">
          <t-switch v-model="formData.expandMutex" />
        </t-form-item>
        <t-form-item label="使用多标签页" name="isUseTabsRouter">
          <t-switch v-model="formData.isUseTabsRouter"></t-switch>
        </t-form-item>
        <t-form-item v-show="formData.layout === 'side' || formData.layout === 'sub'" label="显示顶部" name="showHeader">
          <t-switch v-model="formData.showHeader" />
        </t-form-item>
        <t-form-item label="显示底部" name="showFooter">
          <t-switch v-model="formData.showFooter" />
        </t-form-item>
      </t-form>
      <div v-if="isShowSettingPanel" class="setting-info">
        <p>请复制后手动修改配置文件: /public/static/config/style.json</p>
...
// 通过url判断当前是否是 admin 面板，url以网址+/admin开头的为 admin面板
const isShowSettingPanel = computed(() => {
