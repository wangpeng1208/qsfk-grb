<template>
  <t-card class="basic-container" :bordered="false">
    ...
    <t-form ref="form" class="base-form" :data="formData" :rules="FORM_RULES" label-align="left" :label-width="150"
      scroll-to-first-error="smooth" status-icon @submit="onSubmit">
      <t-form-item label="商品分类" name="cate_id">
        <t-select v-model="formData.cate_id" placeholder="请选择商品分类" size="medium" :style="{ width: '600px' }"
          :options="goodsCategory" />
      </t-form-item>
      <t-form-item label="商品名称" name="name" tips="填写商品名称，长度需在2-100位之间">
        <t-input v-model="formData.name" :maxlength="100" show-limit-number placeholder="请填写商品名称" size="medium"
          :style="{ width: '600px' }" />
      </t-form-item>
      <t-form-item label="价格设置" name="price">
        <t-space style="padding-bottom: 20px;">
          <t-input-number v-model="formData.price" theme="normal" align="center" label="销售价格" suffix="元" tips="销售价格，范围在0.1-20000"
            placeholder="请填写销售价格" :style="{ width: '292px' }" size="medium"></t-input-number>
          <t-input-number v-model="formData.cost_price" theme="normal" align="center" label="成本价格" suffix="元" tips="成本价，方便统计。不得高于售价"
            placeholder="请填写成本价格" :style="{ width: '292px' }" size="medium"></t-input-number>
        </t-space>
      </t-form-item>

      <t-form-item label="限购设置" name="event_give">
        <t-space>
          <t-input-number v-model="formData.limit_quantity" theme="normal" align="center" label="最少购买" suffix="件"
            placeholder="请填写最少购买数量" :style="{ width: '292px' }" size="medium"></t-input-number>
          <t-input-number v-model="formData.limit_quantity_max" theme="normal" align="center" label="最多购买" suffix="件"
            placeholder="请填写最多购买数量" :style="{ width: '292px' }" size="medium"></t-input-number>
        </t-space>
      </t-form-item>
      ...
      <t-form-item v-if="formData.event_give.length" label="" name="event_give" tips="【倍数关系】每买a件，送b件。会展示在购买下单页总数上">
        <t-space direction="vertical">
          <div v-for="(item, i) in formData.event_give" :key="i">
            <t-space>
              <t-input-number v-model="item.num" theme="normal" align="center" label="买满：" suffix="件"
                placeholder="请填写买满数量" :style="{ width: '292px' }"></t-input-number>
              <t-input-number v-model="item.give_num" theme="normal" align="center" label="赠送：" suffix="件"
                placeholder="请填写赠送数量" :style="{ width: '292px' }"></t-input-number>
              <t-button theme="danger" variant="outline" @click="deleteEventGiveNumber(i)">删除</t-button>
              <t-button v-if="formData.event_give.length" variant="outline" theme="primary"
                @click="addEventGiveNumber(1)">
                添加 </t-button>
            </t-space>
          </div>
        </t-space>
      ...
      <t-form-item v-if="formData.addtion_give.length" label="" name="addtion_give"
        tips="【附加关系】若编辑（B商品，2,1）意思为买2个当前商品送B商品1个，取货时才会显示出来">
        <t-space v-if="formData.addtion_give.length" direction="vertical">
          <div v-for="(item, i) in formData.addtion_give" :key="i">
            <t-space>
              ...
              <t-input v-model="item.bug_num" placeholder="请填写购买数量" type="number" theme="normal" :decimal-places="false"
                :min="1" :style="{ width: '138px' }" />
              <t-input v-model="item.give_num" placeholder="请填写赠送数量" type="number" theme="normal"
                :decimal-places="false" :min="1" :style="{ width: '138px' }" />
              <t-button theme="danger" variant="outline" @click="deleteAddtionGiveNumber(i)">删除</t-button>
              <t-button v-if="formData.addtion_give.length" variant="outline" theme="primary"
                @click="addtionGiveNumber(1)">
                添加 </t-button>
            </t-space>
          </div>
        </t-space>
      ...
      <t-form-item label="批发优惠" name="wholesale_discount_list" tips="">
        <wp-check-tag v-model="formData.wholesale_discount_list" :items="TRUEFALSESWITCH"
          @actions="addWholesaleNumber" />
      </t-form-item>
      <t-form-item v-if="formData.wholesale_discount_list.length" label="" name="wholesale_discount_list">
        <t-space direction="vertical">
          <div v-for="(item, i) in formData.wholesale_discount_list" :key="i">
            <t-space>
              <t-input-number v-model="item.num" theme="normal" align="right" label="数量：" suffix="件"
                placeholder="请填写购买数量" :style="{ width: '290px' }"></t-input-number>
              <t-input-number v-model="item.price" theme="normal" align="right" label="价格：" suffix="元"
                placeholder="请填写价格" :style="{ width: '290px' }"></t-input-number>
              <t-button theme="danger" variant="outline" @click="deleteWholesaleNumber(i)">删除</t-button>
              <t-button v-if="formData.wholesale_discount_list.length" variant="outline" theme="primary"
                @click="addWholesaleNumber(1)"> 添加 </t-button>
            </t-space>
          </div>
        </t-space>
      </t-form-item>
      ...
      <t-form-item label="商品轮播图" name="slider_image" tips="建议尺寸：800*800，可拖拽改变图片顺序，默认首张图为主图，最多上传10张">
        <uploadImage :urls="formData.slider_image" title="上传商品图" :style="{
    maxWidth: '100px',
    width: '100px',
    height: '100px',
  }" :maxlength="10" @setPic="setPic" opt />
      </t-form-item>
      <t-form-item label="商品描述" name="content" tips="必填，商品描述会在买家选择商品时展示，建议写上您商品的简介，官网以及常见的问题"
        :style="{ width: '760px' }">
        <wp-editor v-model="formData.content" style="border: 1px solid var(--td-border-level-2-color); width: 100%"
          height="200px" mode="simple" user-type="admin" />
      </t-form-item>
      <!-- :toolbar-config="{
            toolbarKeys: ['color', 'bold', 'italic', 'underline', 'insertLink', 'fontFamily'],
          }" -->
      <t-form-item label="使用说明" name="remark" tips="必填，商品使用说明会在买家购买完成后展示，建议写上您的产品使用说明，售后联系方式等"
        :style="{ width: '760px' }">
        <wp-editor v-model="formData.remark" style="border: 1px solid var(--td-border-level-2-color); width: 100%"
          height="200px" mode="simple" user-type="admin" />
      </t-form-item>

      <t-form-item label="商品排序" name="sort" tips=""> <t-input-number v-model="formData.sort" :min="0" theme="normal"
          placeholder="" :style="{ width: '600px' }" size="medium" /> </t-form-item>

      <t-form-item>
        <t-button theme="primary" class="form-submit-confirm" @click="onSubmit2" :loading="saveLoading2" size="large">
          {{ opt === 'add' ? '添加' : '编辑' }}并返回商品列表
        </t-button>
        <t-button theme="default" class="form-submit-confirm" type="submit" :loading="saveLoading" variant="text"
          size="small"> {{ opt === 'add' ? '添加' : '编辑' }}后继续{{ opt === 'add' ? '添加' : '编辑' }}商品
        </t-button>
