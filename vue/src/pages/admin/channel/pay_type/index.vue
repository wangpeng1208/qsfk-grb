<template>
  <t-card class="basic-container" :bordered="false">
    <template #title>
      支付方式分类
      <t-tooltip content="用于显示选择支付方式；此处和支付授权渠道有关联；为系统预设性，非必要请勿删除" placement="right">
    ...
    <t-base-table class="basic-table" row-key="id" :data="lists" :columns="columns" :hover="lists.length > 0 ? true : false" :header-affixed-top="headerAffixedTop" :pagination="pagination" max-height="auto" table-layout="auto" lazy-load @page-change="onPageChange">
      <template #logo="{ row }">
