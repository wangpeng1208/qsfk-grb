<template>
  <t-dialog v-model:visible="visible" :close-on-overlay-click="false" :destroy-on-close="true" :header="title" :on-confirm="onSubmit">
    <t-form ref="formRef" label-align="left" :data="formData" :rules="FORM_RULES" :label-width="80">
      ...
      <t-form-item label="代码接口" name="code">
        <t-select v-model="formData.code" :options="codeOptions" clearable placeholder="请选择支付代码接口" @change="codeChange" />
      </t-form-item>
      ...
      <t-form-item label="排序" name="sort" help="排序(越大越靠前)">
...
import { add, code, detail, edit } from '@/api/admin/channel/collection';
...
const codeChange = (value: any) => {
  formData.account_fields = codeOptions.value.find((item: any) => item.value === value)?.fields;
};
const options = [
  { label: '启用', value: 1 },
  { label: '禁用', value: 0 },
