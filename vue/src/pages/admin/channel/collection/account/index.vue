<template>
  <t-card title="收款通道账号管理" class="basic-container" :bordered="false">
    ...
    <t-base-table class="basic-table" ref="tableRef" row-key="id" :data="lists" :columns="columns" :hover="lists.length > 0 ? true : false" :header-affixed-top="headerAffixedTop" max-height="auto" table-layout="auto" :pagination="pagination" lazy-load @page-change="onPageChange">
      <template #status="{ row }">
        <t-tag v-if="row.status === 1" theme="success">启用</t-tag>
        <t-tag v-else theme="danger">禁用</t-tag>
      </template>
      <template #userid="{ row }">
        <t-tag v-if="row.user_id === 0">平台所有</t-tag>
        <t-tag v-else>商户ID {{ row.user_id }}</t-tag>
      </template>
