...
class GoodsService
{
    public function sendOut($trade_no)
    {
        ...
        // 已冻结
        if ($goods->is_freeze) {
            throw new \Exception("商品已冻结，请联系客服处理！");
        }
                ...
                try {
                    ...
                    if ($result) {
                        ...
                        if ($order_card_quantity <= $order->quantity) {
                            ...
                            Db::rollback();
                            throw new \Exception("请重试-01");
                        }
                    } else {
                        Db::rollback();

                        throw new \Exception("请重试sss-02");
                    }
                } catch (\Exception $e) {
                    Db::rollback();
    ...
    public function goodsAddtionsCard($list, $order_id)
    {
        foreach ($list as $value) {
            $card_list = Db::name('goods_card')->where(['goods_id' => $value['good_id'], 'status' => 1])->limit($value['give_num'])->select();
